...
def test_company_parser_finds_stafford_phase_rows_and_contact_quality_cases() -> None:
    ...
    assert names["Curtis Rakosi"] == 2
    assert names["Curits Rakosi"] == 1
    assert names["Dan Delforge"] == 3
    by_name = {row.name: row for row in parsed.contacts}
    assert by_name["Brian Owrey"].email == "info@eereed.com"
    assert by_name["Cary Brown"].email == "cbrown@eereedeast.com"
    assert by_name["Renee Fullick"].email == "bz@zapalacreed.com"
