...
from app.resolution.projects import extract_phase_descriptor
...
def _seed(session: Session):
    ...
    return stafford, ProjectAccountResolutionService(session).run()
...
def test_contact_duplicate_clusters_and_recurrence_do_not_imply_authority() -> None:
    with _session() as session:
        _, result = _seed(session)
        assert any(
            x.canonical_name != x.duplicate_name and "FUZZY" in x.match_method
            for x in result.duplicate_people
        )
        assert len(result.duplicate_people) >= 3
        recurrence = result.account_intelligence.recurring_contacts
        assert sorted((row.unique_project_count for row in recurrence), reverse=True)[:3] == [5, 2, 2]
        assert all(row.rental_authority_implied is False for row in recurrence)
...
def test_account_intelligence_reconciles_source_rows_but_reports_unique_projects_and_freshness() -> None:
    with _session() as session:
        ...
        assert acct.source_section_counts == {
            "BIDDING": 0,
            "BIDDING_ROLE": 74,
            "PLANNING": 6,
            "POST_BID": 87,
