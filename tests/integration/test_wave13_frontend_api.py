...
def wave13_state(tmp_path_factory):
    ...
    engine = build_engine(f"sqlite+pysqlite:///{root / 'ui.db'}")
    ...
    with factory() as session:
        ...
        pid = project.id
    return TestClient(
        create_app(session_factory=factory, demo_mode=True, upload_dir=root / "uploads")
    ), pid


def test_project_organization_discovery_supports_frontend_without_uuid_hardcoding(wave13_state):
    client, pid = wave13_state
    body = client.get(f"/api/v1/projects/{pid}/organizations").json()
    gc = [x for x in body["items"] if x["role"] == "General Contractor"]
    assert (
        len(gc) == 1
        and gc[0]["canonical_name"] == "EE Reed Construction"
        and gc[0]["verification_state"] == "SUPPORTED"
    )
    assert body["project_group"]["canonical_name"] == "Stafford Technology Campus"
    assert [x["phase_label"] for x in body["project_group"]["projects"]] == [
        "Phases 1 & 2",
        "Phases 3 & 4",
    ]
...
def test_demo_api_preserves_stafford_truth_and_unknown_authority(wave13_state):
    client, pid = wave13_state
    a = client.get(f"/api/v1/projects/{pid}/assessment").json()
    c = client.get(f"/api/v1/projects/{pid}/contact-candidates").json()
    r = client.get(f"/api/v1/projects/{pid}/crm-readiness").json()
    assert (
        a["assessment"]["disposition"] == "VERIFY"
        and float(a["assessment"]["commercial_fit_score"]) == 57.0
    )
    assert a["assessment"]["overall_band"] == "Promising candidate"
    assert (
        c["items"][0]["display_name"] == "Doug Meadows"
        and c["items"][0]["verification"]["project_association"] == "VERIFIED"
        and c["items"][0]["verification"]["rental_authority"] == "UNKNOWN"
    )
...
def test_demo_api_evidence_has_no_private_server_paths(wave13_state):
    client, pid = wave13_state
    e = client.get(f"/api/v1/projects/{pid}/evidence").json()
    assert e["demo_mode"] is True and e["count"] > 0
...
def test_monday_kpi_does_not_fabricate_outcomes(wave13_state):
    ...
    b = client.get("/api/v1/monday-brief").json()
    assert (
        b["primary_kpi"]["display"] == "N/A"
        and b["primary_kpi"]["status"] == "PRODUCTION_OUTCOME_HISTORY_NOT_CONNECTED"
    )
...
def test_openapi_inventory_includes_wave13_read_only_discovery_projection(wave13_state):
    ...
    assert len(paths) == 37
    assert "/api/v1/portfolio/triage" in paths
    assert "/api/v1/analyst/query/stream" in paths
    assert operations == 38
...
def test_compact_action_status_stacks_at_the_mobile_layout_breakpoint() -> None:
    ...
    assert (
        ".ranked-action.compact > .pill { grid-column: 2; justify-self: start; margin-top: 3px; }"
        in mobile
    )
