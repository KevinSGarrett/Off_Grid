...
function Analyst({ d }: { d: DashboardData }) {
  ...
  return <div className="page" data-view="analyst"><PageHeader eyebrow="Commercial Analyst" title="Evidence-grounded answers" subtitle="Read-only analysis preserves unknowns and cannot mutate deterministic truth." d={d}/><div className="analyst-status"><Pill>Model policy enabled</Pill><Pill>Grounded mode on</Pill><Pill>Read-only</Pill><Pill>Demo mode</Pill></div><section className="analyst-layout"><article className="panel analyst-conversation"><SectionHead title="Analyst conversation" icon={MessageSquareText}/><form onSubmit={submit}><label htmlFor="analyst-question">Ask about Stafford</label><div className="ask"><input id="analyst-question" value={question} onChange={(event) => setQuestion(event.target.value)}/><button className="button primary" disabled={busy}>{busy ? <RefreshCw className="spin"/> : <Sparkles/>}{busy ? "Analyzing…" : "Ask"}</button></div></form>{response ? <div className="answer"><div><Pill>{response.status}</Pill><span>External request: {response.external_request_executed ? "executed" : "not executed"}</span></div>{response.answer ? <pre>{typeof response.answer === "string" ? response.answer : JSON.stringify(response.answer, null, 2)}</pre> : <><h3>Deterministic system remains available.</h3><p>{response.fallback_reason || "The optional model is disabled; core intelligence remains operational."}</p></>}</div> : <div className="answer placeholder"><Sparkles size={28}/><h3>Try a CEO-style challenge.</h3><p>If the optional model is unavailable, this panel reports the fallback instead of inventing an answer.</p></div>}</article><aside className="analyst-aside"><article className="panel"><SectionHead title="Suggested questions" icon={CircleHelp}/>{questions.map((item) => <button className={question === item ? "active" : ""} key={item} onClick={() => setQuestion(item)}><Target size={15}/><span>{item}</span><ChevronRight size={15}/></button>)}</article><article className="panel guardrails"><SectionHead title="Analyst guardrails" icon={ShieldCheck}/><span><LockKeyhole/>No external writes</span><span><FileSearch/>Evidence required</span><span><CircleHelp/>Unknowns preserved</span><span><PackageCheck/>CRM gates remain deterministic</span></article></aside></section></div>;
}
