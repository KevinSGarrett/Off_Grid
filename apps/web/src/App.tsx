...
function PageHeader({ eyebrow, title, subtitle, d, actions }: { eyebrow: string; title: string; subtitle?: string; d?: DashboardData; actions?: ReactNode }) {
  return <header className="page-header">
    ...
    {d && <div className="project-facts" aria-label="Project context">
      ...
      <Pill>{d.assessment.assessment.disposition} / verify</Pill>
    </div>}
...
function ScoreCard({ label, value, note, icon: Icon }: { label: string; value: unknown; note: string; icon?: LucideIcon }) {
  const n = Math.max(0, Math.min(100, Number(value) || 0));
  const kind = n >= 80 ? "good" : n >= 60 ? "warn" : "bad";
  return <article className="score-card"><div className="score-top"><span className={`score-icon ${kind}`}>{Icon && <Icon size={23} />}</span><div><span>{label}</span><strong>{score(value)}</strong></div><b className={kind}>{scoreBand(value)}</b></div><Progress value={n} toneClass={kind}/><small>{note}</small></article>;
}
...
function GuidedReview({ d, start, setView }: { d: DashboardData; start: () => void; setView: (view: ViewKey) => void }) {
  ...
  return <div className="page guided-page" data-view="guided">
    <section className="guided-hero">
      ...
      <article className="opportunity-card"><div className="location"><MapPin size={15}/>{d.project.city}, {d.project.region}</div><h2>{d.project.canonical_name}</h2><div className="recommendation"><span>Recommendation</span><strong>{assessment.disposition} / VERIFY</strong></div><div className="score-triplet"><div><Target/><span>Commercial Fit</span><b>{score(assessment.commercial_fit_score)}</b></div><div><ShieldCheck/><span>Data Confidence</span><b>{score(assessment.data_confidence_score)}</b></div><div><Eye/><span>Evidence Records</span><b>{d.evidence.length}</b></div></div><div className="warning-list"><b>{warnings.length} trust warnings</b>{warnings.map((warning: any) => <button key={warning.id} onClick={() => setView("evidence")}><TriangleAlert size={14}/><span>{warning.title}</span><ChevronRight size={14}/></button>)}</div></article>
    </section>
...
function CommandCenter({ d, setView }: { d: DashboardData; setView: (view: ViewKey) => void }) {
  ...
  return <div className="page" data-view="command">
    ...
    <section className="command-grid">
      <article className="panel command-opportunity"><div className="opportunity-title"><span className="feature-icon"><Building2/></span><div><h2>{d.project.canonical_name}</h2><div className="context-line"><MapPin size={14}/>{d.project.city}, {d.project.region}<span>•</span>{titleCase(d.project.stage)}</div></div><Pill>High priority — {a.disposition} / verify</Pill></div><div className="command-score-row"><ScoreCard label="Commercial Fit" value={a.commercial_fit_score} note="Opportunity attractiveness" icon={Target}/><ScoreCard label="Data Confidence" value={a.data_confidence_score} note="Strength of current source state" icon={ShieldCheck}/><div className="reasons"><b>Key reasons</b>{d.signals.slice(0, 5).map((item: any) => <span key={item.id}><CircleCheck size={15}/>{titleCase(item.key)}</span>)}</div></div></article>
...
function ProjectIntelligence({ d, open }: { d: DashboardData; open: (item: Evidence) => void }) {
  const a = d.assessment.assessment;
  const counterfactual = d.sensitivity.counterfactuals.find((item: any) => item.key === "ignore_reported_value") || {};
  ...
  return <div className="page" data-view="project">
    ...
    <section className="score-band"><ScoreCard label="Commercial Fit" value={a.commercial_fit_score} note="Calculated opportunity fit" icon={Target}/><ScoreCard label="Data Confidence" value={a.data_confidence_score} note="Separate source confidence" icon={ShieldCheck}/>{d.assessment.product_fits.map((fit: any) => <ScoreCard key={fit.product_code} label={`${fit.product_code} Fit`} value={fit.fit_score} note={titleCase(fit.confidence_state)} icon={Zap}/>)}</section>
    <section className="project-grid">
      ...
      <article className="panel challenge"><SectionHead title="Challenge this recommendation" icon={RefreshCw} aside="Backend-generated counterfactual"/><div className="comparison"><div><span>Baseline</span><b>{score(d.sensitivity.baseline.commercial_fit_score)}</b><Pill>{d.sensitivity.baseline.disposition}</Pill></div><ArrowRight/><div><span>Ignore reported value</span><b>{score(counterfactual.score)}</b><Pill>{counterfactual.disposition || "UNKNOWN"}</Pill></div></div><button className="text-button" onClick={() => reportedValue && open(reportedValue)}>Inspect reported {money(d.project.reported_value)} value <ArrowRight size={15}/></button></article>
    </section>
  </div>;
}
...
function ProductFit({ d }: { d: DashboardData }) {
  ...
  return <div className="page" data-view="product"><PageHeader eyebrow="Product Fit" title={d.project.canonical_name} subtitle="Current project signals mapped to the registered Off Grid product portfolio." d={d}/><section className="product-cards">{d.assessment.product_fits.map((fit: any) => { const questions = stringList(fit.missing_evidence); return <article className="panel" key={fit.product_code}><div className="product-heading"><span className="feature-icon"><Zap/></span><div><h2>{fit.product_code}</h2><Pill>{fit.fit_band}</Pill></div><strong>{score(fit.fit_score)}</strong></div><Progress value={fit.fit_score} toneClass={Number(fit.fit_score) >= 80 ? "good" : "warn"}/><p>{fit.explanation}</p><div className="question-box"><b>Open questions</b>{questions.length ? <ul>{questions.map((item) => <li key={item}>{item}</li>)}</ul> : <span>No additional evidence request returned.</span>}</div></article>; })}</section><section className="product-grid"><article className="panel"><SectionHead title="Project-to-product signals" icon={GitBranch}/>{d.signals.slice(0, 7).map((item: any) => <div className="signal-matrix" key={item.id}><div><b>{titleCase(item.key)}</b><small>{item.explanation || item.value}</small></div><Pill>{item.classification}</Pill>{d.assessment.product_fits.map((fit: any) => <span key={fit.product_code} title={`${fit.product_code} fit ${score(fit.fit_score)}`} className={Number(fit.fit_score) >= 80 ? "on" : "mid"}/>)}</div>)}</article><article className="panel"><SectionHead title="What we still need to verify" icon={CircleHelp}/>{openQuestions.slice(0, 8).map((item) => <div className="question-row" key={item}><CircleHelp size={17}/><span>{item}</span><ChevronRight size={15}/></div>)}</article><article className="panel"><SectionHead title="Product fit comparison" icon={BarChart3}/>{d.assessment.product_fits.map((fit: any) => <div className="comparison-row" key={fit.product_code}><b>{fit.product_code}</b><Progress value={fit.fit_score} toneClass={Number(fit.fit_score) >= 80 ? "good" : "warn"}/><strong>{score(fit.fit_score)}</strong><Pill>{fit.confidence_state}</Pill></div>)}</article></section></div>;
}
...
function Analyst({ d }: { d: DashboardData }) {
  ...
  async function submit(event?: FormEvent) {
    ...
    try { setResponse(await askAnalyst(d.project.id, question)); }
  ...
  return <div className="page" data-view="analyst"><PageHeader eyebrow="Commercial Analyst" title="Evidence-grounded answers" subtitle="Read-only analysis preserves unknowns and cannot mutate deterministic truth." d={d}/><div className="analyst-status"><Pill>OpenAI available</Pill><Pill>Grounded mode on</Pill><Pill>Read-only</Pill><Pill>Demo mode</Pill></div><section className="analyst-layout"><article className="panel analyst-conversation"><SectionHead title="Analyst conversation" icon={MessageSquareText}/><form onSubmit={submit}><label htmlFor="analyst-question">Ask about Stafford</label><div className="ask"><input id="analyst-question" value={question} onChange={(event) => setQuestion(event.target.value)} disabled={busy}/><button className="button primary" disabled={busy || question.trim().length < 3}>{busy ? <RefreshCw className="spin"/> : <Sparkles/>}{busy ? "Analyzing…" : "Ask"}</button></div></form>{response ? <div className="answer" aria-live="polite"><div className="answer-status"><Pill>{response.status}</Pill><span>{response.external_request_executed ? "OpenAI request executed" : "Deterministic fallback"}</span>{response.model_id && <span>{response.model_id}</span>}{response.grounding?.status && <Pill>{response.grounding.status} grounding</Pill>}</div>{answer ? <div className="analyst-result"><h3>Analyst answer</h3><p className="analyst-answer-copy">{typeof answer === "string" ? answer : answer.answer}</p>{Array.isArray(answer.claims) && answer.claims.length > 0 && <section className="analyst-claims"><h4>Grounded claims</h4>{answer.claims.map((claim: any) => <article key={claim.claim_id}><div><Pill>{claim.classification}</Pill><span>{titleCase(claim.claim_type)}</span></div><p>{claim.claim_text}</p><small>{claim.evidence_ids?.length || 0} cited evidence record{claim.evidence_ids?.length === 1 ? "" : "s"}</small></article>)}</section>}{Array.isArray(answer.unknowns) && answer.unknowns.length > 0 && <section className="analyst-unknowns"><h4>Still unknown</h4><ul>{answer.unknowns.map((item: string) => <li key={item}>{item}</li>)}</ul></section>}<div className="answer-foot"><span>{answer.tool_calls_used?.length || 0} read-only tools used</span><span>Estimated request cost ${response.estimated_cost_usd || "0"}</span></div></div> : <div className="fallback-answer"><h3>Deterministic system remains available.</h3><p>{response.fallback_reason || "The optional model is unavailable; core intelligence remains operational."}</p></div>}</div> : <div className="answer placeholder"><Sparkles size={28}/><h3>Try a CEO-style challenge.</h3><p>The analyst will use selected evidence and read-only tools. If the provider is unavailable, this panel reports the fallback instead of inventing an answer.</p></div>}</article><aside className="analyst-aside"><article className="panel"><SectionHead title="Suggested questions" icon={CircleHelp}/>{questions.map((item) => <button className={question === item ? "active" : ""} key={item} onClick={() => setQuestion(item)}><Target size={15}/><span>{item}</span><ChevronRight size={15}/></button>)}</article><article className="panel guardrails"><SectionHead title="Analyst guardrails" icon={ShieldCheck}/><span><LockKeyhole/>No external writes</span><span><FileSearch/>Evidence required</span><span><CircleHelp/>Unknowns preserved</span><span><PackageCheck/>CRM gates remain deterministic</span></article></aside></section></div>;
}

function MondayBrief({ d, setView }: { d: DashboardData; setView: (view: ViewKey) => void }) {
  const pipeline = Object.entries(d.monday.pipeline);
  return <div className="page" data-view="monday"><PageHeader eyebrow="Executive operating rhythm" title="Monday Morning Brief" subtitle="A truthful weekly view of pipeline, opportunity, and attention required." d={d}/><section className="headline-kpi"><span className="kpi-icon"><BarChart3/></span><div><p className="eyebrow">Headline KPI</p><h2>System-sourced demos booked — rolling 30 days</h2></div><strong>{d.monday.primary_kpi.display}</strong><p>Production outcome history is not connected in this environment.</p><Pill>Intended KPI</Pill></section><section className="pipeline-band">{pipeline.map(([key, value], index) => <div key={key}><span className="pipeline-icon">{index === pipeline.length - 1 ? <CalendarDays/> : index < 2 ? <Database/> : <UsersRound/>}</span><span><b>{titleCase(key)}</b><strong>{String(value)}</strong></span>{index < pipeline.length - 1 && <ChevronRight/>}</div>)}</section><section className="monday-grid"><article className="panel top-opportunity"><SectionHead title="Top opportunity" icon={Building2}/><h2>{d.monday.top_opportunity?.name || "No qualified opportunity"}</h2>{d.monday.top_opportunity ? <><div className="context-line"><MapPin size={14}/>{d.project.city}, {d.project.region}<span>•</span>{titleCase(d.project.stage)}</div><div className="twoscores"><div><span>Commercial fit</span><b>{score(d.monday.top_opportunity.commercial_fit)}</b></div><div><span>Data confidence</span><b>{score(d.monday.top_opportunity.data_confidence)}</b></div></div><Pill>{d.monday.top_opportunity.disposition}</Pill><button className="text-button" onClick={() => setView("project")}>View full intelligence <ArrowRight size={15}/></button></> : <Empty title="No opportunity returned" detail="The backend has no current top opportunity."/>}</article><article className="panel attention"><SectionHead title="Attention required" icon={TriangleAlert}/>{d.monday.attention_required.length ? d.monday.attention_required.map((item: any) => <div className="attention-row" key={item.id}><span className="alert-icon"><TriangleAlert size={16}/></span><div><b>{item.summary}</b><small>{item.detail || "Validation work required"}</small></div><Pill>{item.status}</Pill><span>P{item.priority}</span></div>) : d.quality.slice(0, 5).map((item: any) => <div className="attention-row" key={item.id}><span className="alert-icon"><TriangleAlert size={16}/></span><div><b>{item.title}</b><small>{item.decision_impact || item.detail}</small></div><Pill>{item.severity}</Pill><span>Verify</span></div>)}<button className="text-button" onClick={() => setView("exceptions")}>View exception queue <ArrowRight size={15}/></button></article><article className="panel weekly"><SectionHead title="This week in the pipeline" icon={BarChart3}/>{pipeline.map(([key, value]) => <div className="weekly-row" key={key}><span className="row-icon"><CircleCheck size={16}/></span><b>{titleCase(key)}</b><strong>{String(value)}</strong></div>)}<p className="note">These are current system diagnostics, not claimed production conversions.</p></article></section></div>;
}
