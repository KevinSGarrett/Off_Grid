...
import type { AnalystResponse, CRMRequest, DashboardData, Evidence } from "./types";
...
function metricLabel(d: DashboardData, key: string) {
  return d.monday.metric_definitions[key]?.label || titleCase(key);
}
...
function CommandCenter({ d, setView }: { d: DashboardData; setView: (view: ViewKey) => void }) {
  ...
  return <div className="page" data-view="command">
    ...
    <section className="command-grid">
      ...
      <article className="panel trust-panel"><SectionHead title={`Top ${Math.min(3, d.quality.length)} of ${d.quality.length} trust issues`} icon={ShieldCheck}/>{d.quality.slice(0, 3).map((item) => <button key={item.id} onClick={() => setView("evidence")}><span className="alert-icon"><TriangleAlert size={18}/></span><div><b>{item.title}</b><p>Decision impact: {titleCase(item.decision_impact || "Not specified")}</p></div><ChevronRight size={16}/></button>)}</article>
      <article className="panel funnel"><SectionHead title="Current system diagnostics" icon={BarChart3}/><div className="funnel-bars">{Object.entries(d.monday.pipeline).map(([key, value]) => <div key={key}><b>{String(value)}</b><span>{metricLabel(d, key)}</span></div>)}</div><p className="note">{d.monday.pipeline_semantics}</p></article>
...
function ProjectIntelligence({ d, open }: { d: DashboardData; open: (item: Evidence) => void }) {
  ...
  return <div className="page" data-view="project">
    ...
    <section className="project-grid">
      ...
      <article className="panel contact-summary"><SectionHead title="Contact resolution" icon={UsersRound}/>{d.candidates.items.slice(0, 4).map((person) => <div className="table-row contact-row" key={person.candidate_id}><span className="avatar">{String(person.display_name).split(" ").map((part: string) => part[0]).slice(0, 2).join("")}</span><div><b>{person.display_name}</b><small>{titleCase(person.target_persona)}</small></div><Pill>{person.verification?.rental_authority || "UNKNOWN"}</Pill><span className="candidate-priority"><b>{score(person.candidate_score)}</b><small>Investigation priority</small></span></div>)}</article>
      ...
      <article className="panel first-call"><SectionHead title="First-call kit" aside={d.actions.first_call_kit.version} icon={MessageSquareText}/><ol>{d.actions.first_call_kit.questions.map((item: string) => <li key={item}>{item}</li>)}</ol></article>
...
function ExceptionQueue({ d }: { d: DashboardData }) {
  ...
  const warnings = filter === "ALL" ? d.quality : d.quality.filter((item) => item.severity.toUpperCase() === filter);
  return <div className="page" data-view="exceptions"><PageHeader eyebrow="Pipeline Failure & Recovery" title="Exception Queue" subtitle="Workflow exceptions and evidence-quality warnings are separate operational concepts." d={d}/><section className="score-band four"><Metric label="Open workflow exceptions" value={d.exceptions.count} note="Pipeline work records only" icon={TriangleAlert} kind={d.exceptions.count ? "bad" : "good"}/><Metric label="Quality warnings requiring review" value={d.quality.filter((item) => item.review_status === "NEEDS_REVIEW").length} note="Evidence findings, not exceptions" icon={Flag} kind="warn"/><Metric label="Progression-blocking quality warnings" value={d.quality.filter((item) => item.blocks_progression).length} icon={LockKeyhole} kind="bad"/><Metric label="Resolved today" value="N/A" note="Resolution history not connected" icon={CircleCheck} kind="warn"/></section><section className="panel exception-panel"><SectionHead title="Workflow exceptions" aside={`${d.exceptions.count} current`} icon={TriangleAlert}/>{d.exceptions.items.length ? <div className="table-scroll"><div className="table-head exception-grid"><span>Priority</span><span>Exception</span><span>Reason</span><span>Status</span><span>Next action</span></div>{d.exceptions.items.map((item) => <article className="exception-grid" key={item.id}><Pill>{item.priority}</Pill><div><b>{item.summary}</b><small>Workflow exception</small></div><span>{item.detail || "No additional detail recorded"}</span><Pill>{item.status}</Pill><span>Follow the recorded resolution action</span></article>)}</div> : <Empty title="No open workflow exceptions" detail="Quality warnings may still require review below; zero exceptions does not mean the evidence is issue-free."/>}</section><section className="panel exception-panel quality-review-panel"><SectionHead title="Quality review items" aside={`${d.quality.length} current`} icon={Flag}/><div className="filter-bar" aria-label="Quality warning filters">{["ALL","CRITICAL","HIGH","MEDIUM","LOW"].map((value) => <button key={value} className={filter === value ? "active" : ""} onClick={() => setFilter(value)}>{titleCase(value)} <span>{value === "ALL" ? d.quality.length : d.quality.filter((item) => item.severity.toUpperCase() === value).length}</span></button>)}</div>{warnings.length ? <div className="table-scroll"><div className="table-head exception-grid"><span>Severity</span><span>Warning</span><span>Decision impact</span><span>Review status</span><span>Next action</span></div>{warnings.map((item) => <article className="exception-grid" key={item.id}><Pill>{item.severity}</Pill><div><b>{item.title}</b><small>{item.blocks_progression ? "Progression blocker" : "Evidence review"}</small></div><span>{titleCase(item.decision_impact || "Not specified")}: {item.detail}</span><Pill>{item.review_status}</Pill><span>{item.recommended_action}</span></article>)}</div> : <Empty title="No quality warnings match this filter" detail="Choose another severity filter to continue reviewing evidence findings."/>}</section></div>;
}

function crmRequestIdentity(request: CRMRequest) {
  return request.body.name || request.body.title || "Identity not present in this safe preview";
}

function CrmPreview({ d, setView }: { d: DashboardData; setView: (view: ViewKey) => void }) {
  return <div className="page" data-view="crm"><PageHeader eyebrow="CRM Preview · read-only" title="CRM Lead record ready · Deal blocked" subtitle="Lead-record readiness is not outreach authority or Deal readiness. No data is written to Pipedrive in this preview." d={d}/><div className="dry-run-banner"><Info size={18}/><span><b>DRY RUN PREVIEW</b> — current API state only; external write authority remains server-side.</span><Pill>{d.crm.pipedrive.mode || "PREVIEW"}</Pill></div><section className="crm-layout"><article className="panel readiness"><SectionHead title="CRM readiness" icon={PackageCheck}/>{[["Project qualified",true],["CRM Lead record ready",d.readiness.lead_ready],["Deal ready",d.readiness.deal_ready],["Evidence retained",true]].map(([label,ready]) => <div key={String(label)} className={ready ? "pass" : "blocked"}>{ready ? <CircleCheck/> : <TriangleAlert/>}<div><b>{String(label)}</b><small>{ready ? "Deterministic record gate passed; no outreach sent" : "Commercial validation still required"}</small></div></div>)}<div className="readiness-outcome"><span>Permitted CRM promotion</span><Pill>{d.readiness.permitted_promotion}</Pill></div></article><article className="panel crm-requests"><SectionHead title="Pipedrive request preview" icon={Database}/><div className="request-grid">{d.crm.pipedrive.requests.map((request) => <article key={request.canonical_key || request.label}><div><Pill>{request.status}</Pill><span>{titleCase(request.object_type)}</span></div><h3>{request.label}</h3><b>{crmRequestIdentity(request)}</b><p>{request.blocked_reason || "Validated preview; no live write executed."}</p><details><summary>Inspect request contract and demo-safe body</summary><dl><div><dt>Canonical key</dt><dd>{request.canonical_key || "Not applicable"}</dd></div><div><dt>Request</dt><dd>{request.method} {request.path}</dd></div><div><dt>Dependencies</dt><dd>{request.dependencies.length ? request.dependencies.join(", ") : "None"}</dd></div></dl><pre>{JSON.stringify(request.body, null, 2)}</pre><small>Canonical keys make an authorized retry idempotent; this preview performs no retry or write.</small></details></article>)}</div></article><aside className="crm-aside"><article className="panel blocker-panel"><SectionHead title="Why Deal creation is blocked" icon={LockKeyhole}/>{d.readiness.deal_blockers.map((item) => <div className="blocker" key={item}><TriangleAlert size={16}/><b>{titleCase(item)}</b></div>)}<button className="button secondary" onClick={() => setView("contacts")}>Open Contact Resolution</button></article><article className="panel"><SectionHead title="Duplicate prevention" icon={ShieldCheck}/><div className="success-callout"><CircleCheck/><span>No live duplicates can be created in dry-run mode.</span></div><div className="safety"><div><span>External writes</span><b>{String(d.crm.external_writes_executed)}</b></div><div><span>Mode</span><Pill>{d.crm.pipedrive.mode || "PREVIEW"}</Pill></div></div></article></aside></section></div>;
}

function CommercialMotion({ d, setView }: { d: DashboardData; setView: (view: ViewKey) => void }) {
  return <div className="page" data-view="commercial"><PageHeader eyebrow="Commercial Motion" title="Stafford opportunity" subtitle="Contractor demand and rental-house supply are linked, but they remain separate motions." d={d}/><section className="motion-paths">{d.motions.items.map((motion) => { const contractor = motion.motion_type === "CONTRACTOR"; return <article className="panel" key={motion.id}><SectionHead title={contractor ? "Contractor demand motion" : "Rental house / fleet motion"} icon={contractor ? Building2 : Handshake}/><div className="motion-flow neutral-map">{motion.dependency_map.map((step, index) => <div key={step.source}><span className={tone(step.state)}>{index + 1}</span><b>{step.label}</b><Pill>{step.state}</Pill>{index < motion.dependency_map.length - 1 && <ArrowRight/>}</div>)}</div><div className="motion-status"><Pill>{motion.status}</Pill><Pill>{motion.confidence_state}</Pill><p>{motion.summary}</p></div></article>; })}</section><section className="commercial-grid"><article className="panel"><SectionHead title="Current motion state" icon={Gauge}/>{d.motions.items.map((motion) => <div className="motion-meter" key={motion.id}><div><b>{titleCase(motion.motion_type)} motion</b><Pill>{motion.status}</Pill></div><small>{motion.demand_display}</small></div>)}</article><article className="panel"><SectionHead title="Decision support" icon={Lightbulb}/><ul className="check-list"><li>EE Reed is the supported general contractor relationship.</li><li>Current project characteristics support contractor-first investigation.</li><li>Rental partner, authority, and current product need remain unresolved.</li><li>Deal progression cannot outrun verified dependencies.</li></ul></article><article className="panel"><SectionHead title="Next commercial actions" icon={Target}/>{d.actions.items.slice(0, 6).map((item, index) => <div className="ranked-action compact" key={item.id}><span>{index + 1}</span><div><b>{titleCase(item.action_type)}</b><p>{item.reason}</p></div><Pill>{item.status}</Pill></div>)}</article><article className="panel span-2"><SectionHead title="Why contractor-side validation comes first" icon={CircleCheck}/><div className="proof-grid"><span><Check/>Supported GC relationship</span><span><Check/>Active project evidence</span><span><Check/>Project-characteristic relevance signals</span><span><Check/>Dependency-aware actions</span><span><TriangleAlert/>Rental authority unresolved</span><span><TriangleAlert/>Current lighting/power need unresolved</span></div></article><article className="panel"><SectionHead title="Canonical first-call kit" aside={d.actions.first_call_kit.version} icon={MessageSquareText}/><ol className="numbered-list">{d.actions.first_call_kit.questions.map((item) => <li key={item}>{item}</li>)}</ol></article><article className="panel"><SectionHead title="Commercial handoff" icon={Handshake}/><button className="handoff-link" onClick={() => setView("crm")}><PackageCheck/>CRM Preview <ArrowRight/></button><button className="handoff-link" onClick={() => setView("monday")}><CalendarDays/>Monday Brief <ArrowRight/></button></article></section></div>;
}
...
function Analyst({ d }: { d: DashboardData }) {
  ...
  async function submit(event?: FormEvent) {
    ...
    try { const next = await askAnalyst(d.project.id, question, mode, conversation); setResponse(next); if (next.answer && ["SUCCEEDED", "PARTIAL_VALIDATED"].includes(String(next.status))) setConversation((rows) => [...rows, { question, answer: next.answer!.direct_conclusion, claim_ids: next.answer!.claims?.map((claim) => claim.claim_id) || [] }].slice(-4)); }
...
function MondayBrief({ d, setView }: { d: DashboardData; setView: (view: ViewKey) => void }) {
  ...
  const a = d.assessment.assessment;
  return <div className="page" data-view="monday"><PageHeader eyebrow="Executive operating rhythm" title="Monday Morning Brief" subtitle="A truthful weekly view of pipeline, opportunity, and attention required." d={d}/><section className="headline-kpi"><span className="kpi-icon"><BarChart3/></span><div><p className="eyebrow">Headline KPI</p><h2>{d.monday.primary_kpi.name}</h2></div><strong>{d.monday.primary_kpi.display}</strong><p>{d.monday.primary_kpi.interpretation}</p><Pill>{d.monday.primary_kpi.status === "AVAILABLE" ? "Available" : "Not connected"}</Pill></section><section className="pipeline-band">{pipeline.map(([key, value], index) => <div key={key}><span className="pipeline-icon">{index === pipeline.length - 1 ? <CalendarDays/> : index < 2 ? <Database/> : <UsersRound/>}</span><span><b>{metricLabel(d, key)}</b><strong>{String(value)}</strong></span>{index < pipeline.length - 1 && <ChevronRight/>}</div>)}</section><p className="pipeline-semantics">{d.monday.pipeline_semantics}</p><section className="monday-grid"><article className="panel top-opportunity"><SectionHead title="Top opportunity" icon={Building2}/><h2>{d.monday.top_opportunity?.name || "No qualified opportunity"}</h2>{d.monday.top_opportunity ? <><div className="context-line"><MapPin size={14}/>{d.project.city}, {d.project.region}<span>•</span>{titleCase(d.project.stage)}</div><div className="twoscores"><div><span>Commercial fit</span><b>{a.overall_band}</b></div><div><span>Data confidence</span><b>{titleCase(a.confidence_state)}</b></div></div><Pill>{a.operational_action}</Pill><p className="assessment-disclaimer">Deterministic decision support; not a probability or validated demand.</p><button className="text-button" onClick={() => setView("project")}>View full intelligence <ArrowRight size={15}/></button></> : <Empty title="No opportunity returned" detail="The backend has no current top opportunity."/>}</article><article className="panel attention"><SectionHead title="Attention required" icon={TriangleAlert}/>{d.monday.attention_required.length ? d.monday.attention_required.map((item) => <div className="attention-row" key={item.id}><span className="alert-icon"><TriangleAlert size={16}/></span><div><b>{item.summary}</b><small>{item.recommended_action}</small></div><Pill>{item.status}</Pill><span>P{item.priority}</span></div>) : d.quality.map((item) => <div className="attention-row" key={item.id}><span className="alert-icon"><TriangleAlert size={16}/></span><div><b>{item.title}</b><small>{item.recommended_action}</small></div><Pill>{item.review_status}</Pill><span>{titleCase(item.severity)}</span></div>)}<button className="text-button" onClick={() => setView("exceptions")}>View exception queue <ArrowRight size={15}/></button></article><article className="panel weekly"><SectionHead title="Current demo snapshot" icon={BarChart3}/>{pipeline.map(([key, value]) => <div className="weekly-row" key={key}><span className="row-icon"><CircleCheck size={16}/></span><b>{metricLabel(d, key)}</b><strong>{String(value)}</strong></div>)}<p className="note">{d.monday.pipeline_semantics}</p></article></section></div>;
}
...
function UtilityBar({ view, navigate, openNav }: { view: ViewKey; navigate: (view: ViewKey) => void; openNav: () => void }) {
  ...
  return <header className="utility-bar"><button className="mobile-menu icon-button" onClick={openNav} aria-label="Open navigation"><Menu/></button><div className="mobile-logo"><Logo/></div><div className="search-box"><Search size={18}/><label className="sr-only" htmlFor="global-search">Search application views</label><input id="global-search" placeholder="Search application views…" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && matches[0]) { navigate(matches[0].key); setQuery(""); } }}/>{query && <div className="search-results">{matches.length ? matches.map((item) => <button key={item.key} onClick={() => { navigate(item.key); setQuery(""); }}>{item.label}<ArrowRight size={14}/></button>) : <span>No matching application view</span>}</div>}</div><div className="utility-actions"><Pill>Demo mode</Pill><button className="icon-button" aria-label="Notifications unavailable" title="No live notification service" disabled><Bell size={19}/></button><button className="icon-button" aria-label="Open Guided CEO Review help" title="Open Guided CEO Review" onClick={() => navigate("guided")}><CircleHelp size={19}/></button><div className="account-button" aria-label="Demo account identity"><span>OG</span><b>Off Grid</b></div></div><span className="sr-only" aria-live="polite">Current view: {view}</span></header>;
}
