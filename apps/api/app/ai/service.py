...
class OpenAIIntelligenceService:
    ...
    def _validated_analyst_answer(
        ...
        *,
        ...
        def display_claim_text(claim: GroundedClaim) -> str:
        ...
        if supported:
            paragraphs = [
                f"{display_claim_text(claim)}\nWhy: {claim.rationale}"
                for claim in supported
            ]
            answer = "\n\n".join(paragraphs)
            conclusion = display_claim_text(supported[0])
        else:
        ...
        def normalized_claim_type(claim: GroundedClaim) -> str:
            return re.sub(r"[^A-Z0-9]+", "_", claim.claim_type.upper()).strip("_")
        ...
        def is_action_claim(claim: GroundedClaim) -> bool:
            claim_type = normalized_claim_type(claim)
            claim_type_tokens = set(claim_type.split("_"))
            return (
                any(marker in claim_type for marker in action_type_markers)
                or "ACTION" in claim_type_tokens
                or "DISCOVERY" in claim_type_tokens
                or "VERIFICATION" in claim_type_tokens
                or any(token.startswith("RECOMMEND") for token in claim_type_tokens)
            )
        ...
        next_action = display_claim_text(action_claim) if action_claim is not None else conclusion
        trigger_claims = [
            claim
            for claim in supported
            if is_action_claim(claim)
            or any(
                token in normalized_claim_type(claim)
                for token in ("GATE", "REQUIREMENT", "TRIGGER")
            )
        ]
        return parsed.model_copy(
            update={
                ...
                "caveats": list(dict.fromkeys(display_claim_text(claim) for claim in caveat_claims)),
                "counterevidence_and_conflicts": [
                    display_claim_text(claim)
                    for claim in claims
                    if claim.classification == "CONFLICTED"
                ],
                "decision_changing_unknowns": list(
                    dict.fromkeys(display_claim_text(claim) for claim in caveat_claims)
                ),
                "recommendation_triggers": list(
                    dict.fromkeys(display_claim_text(claim) for claim in trigger_claims)
                ),
                "next_action": next_action,
                "claims": claims,
                "unknowns": list(
                    dict.fromkeys(
                        parsed.unknowns + [display_claim_text(claim) for claim in unknown_claims]
                    )
                ),
            }
        )
