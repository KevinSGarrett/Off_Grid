...
# Only provenance values minted by a connected Off Grid pipeline or CRM event are eligible
# for the system-sourced KPI. Free-form/manual, fixture and referral labels do not qualify.
# The suffix is the upstream event/record identifier and must be non-empty.
SYSTEM_SOURCED_OUTCOME_PREFIXES: Final[tuple[str, ...]] = (
    "offgrid_pipeline:",
    "pipedrive:",
)


def _system_source_clause() -> sa.ColumnElement[bool]:
    return sa.or_(
        *(
            sa.and_(
                CommercialOutcome.source.startswith(prefix, autoescape=True),
                sa.func.length(CommercialOutcome.source) > len(prefix),
            )
            for prefix in SYSTEM_SOURCED_OUTCOME_PREFIXES
        )
    )
...
def build_employer_metrics(
    ...
    eligible_outcome_history = session.scalar(
        sa.select(sa.func.count())
        .select_from(CommercialOutcome)
        .join(Project, Project.id == CommercialOutcome.project_id)
        ...
        .where(
            Project.is_synthetic.is_(False),
            CommercialMotion.project_id == CommercialOutcome.project_id,
            _system_source_clause(),
        )
    ) or 0
    ...
    return {
        ...
        "primary_kpi": {
            ...
            "value": int(demos_30d) if eligible_outcome_history else None,
            "display": str(int(demos_30d)) if eligible_outcome_history else "N/A",
            "status": (
                "AVAILABLE" if eligible_outcome_history else "PRODUCTION_OUTCOME_HISTORY_NOT_CONNECTED"
            ),
            "definition": "Observed DEMO_BOOKED outcomes for real projects and matching commercial motions in the inclusive past-30-day window, with a non-empty event identifier in an approved system provenance namespace; manual, fixture, referral, synthetic, unlinked and future observations are excluded.",
