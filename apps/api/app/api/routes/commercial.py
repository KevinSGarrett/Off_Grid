...
def get_actions(project_id: UUID, session: Session = Depends(get_session)) -> dict[str, object]:
    _ensure_project(session, project_id)
    rows = session.scalars(sa.select(NextAction).where(NextAction.project_id == project_id).order_by(NextAction.priority.desc(), NextAction.created_at)).all()
    return {
...
def get_commercial_motions(project_id: UUID, session: Session = Depends(get_session)) -> dict[str, object]:
    _ensure_project(session, project_id)
