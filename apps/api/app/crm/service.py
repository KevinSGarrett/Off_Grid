...
class CommercialIntegrationService:
    ...
    def run(self, project_external_id: str) -> CommercialIntegrationResult:
        ...
        self.session.commit()
        return CommercialIntegrationResult(
    ...
    def _pipedrive_preview(self, project: Project, readiness: CRMReadinessResult) -> PipedrivePreview:
        ...
        return PipedrivePreview(
            ...
            external_writes_executed=0,
            notes=(
                "Pipedrive is dry-run/preview only; no Off Grid credentials were used.",
    ...
    def _audit(self, project: Project, action: str, safe_metadata: dict) -> None:
        ...
        existing = self.session.scalar(
            sa.select(AuditEvent).where(
                ...
                AuditEvent.actor_id == "commercial-integration-service",
        ...
        if existing is None:
            self.session.add(
                AuditEvent(
                    actor_type="SYSTEM",
                    actor_id="commercial-integration-service",
                    action=action,
                    object_type="PROJECT",
                    object_id=str(project.id),
                    reason="Commercial integration preview/contract generation; no external mutation executed.",
