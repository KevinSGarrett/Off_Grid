...
class CommercialPipelineOrchestrator:
    """Thin orchestration over owner-wave services.

    Wave 12 deliberately does not reimplement domain rules in HTTP handlers. Ingestion and
    ...
    def refresh_project(self, project_id: UUID) -> tuple[StageResult, ...]:
        ...
        # CRM preview depends on contact/commercial-motion owner-wave state. It is safe to refresh
        # only when that state already exists; it never performs an external write.
        try:
            crm = Wave10IntegrationService(self.session).run(project.external_id or "")
